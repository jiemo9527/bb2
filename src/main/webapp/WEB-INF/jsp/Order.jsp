		...
		<div class="public_m1">
			...
			<div class="public_m4">
				...
				<c:forEach items="${orders}" var="orders">
					...
					<!-- 判断是否是删除过的订单 -->
					<c:choose>
						<c:when test="${orders.orderStat=='5'}">
									...
									<div class="movie_box">
										<ul class="wjdc_list">
											<div class='kzqy_czbut'>
												<c:choose>
												<c:when test="${orders.orderStat=='1'}">
													
													<a href='/cancelOrder.do?orderID=${orders.orderID}' onClick="return confirm('联系客服说明情况')" class='xy'>取消订单</a>
												</c:when>
													<c:when test="${ordertable.orderStat=='2'}">
														<a href='#' onClick="return confirm('是否确认收货?')" class='sy'>确认收货</a>
													<a href='/cancelOrder.do?orderID=${orders.orderID}' onClick="return confirm('联系客服说明情况')" class='xy'>取消订单</a>
													<a href='#' onClick="return confirm('请打开QQ?')" class='bianji'>联系客服</a>
													</c:when>
												</c:choose>
													<c:choose>
													<c:when test="${orders.orderStat=='3'}">
													<a href='#' onClick="return confirm('请打开QQ?')" class='bianji'>联系客服</a>
													</c:when>
												</c:choose>
												<c:choose>
													<c:when test="${orders.orderStat=='4'}">
														<a href='/delOrder.do?orderID=${orders.orderID}' onClick="return confirm('是否删除此订单?')"
															class='del'>删除</a>
											...
											<li>
												订单名称：               ${orders.orderCName}
											</li>
											<li>
												订单价格：			     ￥${orders.orderPrice}
											</li>

											<li>
												收货地址:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${orders.orderAddr}
											</li>
											<li><b style="color: red">订单状态:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
													<c:choose>
														...
														<c:when test="${orders.orderStat=='4'}">订单完成/退货成功</c:when>
														<%--      <c:otherwise>
         									订单完成
         						</c:otherwise>
         			 --%>
